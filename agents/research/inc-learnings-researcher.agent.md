...
content-search: pattern="module:.*(skill-design|agent-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
When the project ships an authoritative schema file for learning entries, read it on demand for the full contract, including `component` and `root_cause` enums (those are repo-specific and evolve — do not hard-code them here).
...
When no relevant learnings are found, say so explicitly and include the search context so the caller can see what was looked for — the absence is itself useful signal, and the caller's work may be worth capturing as a new learning after it lands.
...
- `/inc-review` — to surface prior learnings relevant to the change under review

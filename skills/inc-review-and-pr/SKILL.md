...
The chain: **review gate → commit-push-PR → stop.** `inc-commit-push-pr` now owns the post-open work — it watches CI + AI reviewers and auto-resolves feedback in a loop before returning. If any stage blocks, surface the result and stop; the user resumes manually.
...
Default to **Tier 1 — `inc-review`** (fast single-agent pass). Escalate to **Tier 2 — `inc-review-deep`** (persona fan-out + dedup) if ANY trigger holds:
...
Announce the chosen tier and the trigger in one line before running (e.g. `Tier 2 (inc-review-deep): diff touches auth middleware.`).
...
Run [inc-review](../inc-review/SKILL.md) or [inc-review-deep](../inc-review-deep/SKILL.md) on the working tree using native invocation or the inline composition rule above.
...
Run [inc-commit-push-pr](../inc-commit-push-pr/SKILL.md) using native invocation or by reading and following that file inline.
...
Print the PR URL and `Ready for human merge.` Do **not** run `inc-merge-pr`.

## Interaction discipline

This skill itself has no blocking prompts — the watch + auto-resolve loop lives inside `inc-commit-push-pr`, which surfaces its own forks (CI failure, `needs-human`, preview failure) and stops. If anything blocks (review-gate stop in Step 1, or commit-push-pr returning short), surface the result and stop cleanly — the user resumes manually from that step.

...
- **Learnings & Past Solutions section** -- results from inc-learnings-researcher, with links to docs/solutions/ files
- **Agent-Native Gaps section** -- results from inc-agent-native-reviewer. Omit if no gaps found.
- **Schema Drift Check section** -- results from inc-schema-drift-detector. Omit if the agent did not run.
- **Deployment Notes section** -- key checklist items from inc-deployment-verification-agent. Omit if the agent did not run.

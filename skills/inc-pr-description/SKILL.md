...
Generate a conventional-commit-style title and a product-first body describing a pull request's work. Returns structured `{title, body_file}` for the caller to apply — this skill never invokes `gh pr edit` or `gh pr create`, and never prompts for interactive confirmation.
...
- `inc-pr-description` → current-branch, no focus, auto-detect base
- `inc-pr-description emphasize the benchmarks` → current-branch, focus = "emphasize the benchmarks"
- `inc-pr-description base:origin/develop` → current-branch, base pinned to `origin/develop`
- `inc-pr-description base:origin/develop emphasize perf` → same + focus
- `inc-pr-description pr:561` → PR #561, no focus
- `inc-pr-description #561 do a good job with the perf story` → PR #561, focus = "do a good job with the perf story"
- `inc-pr-description https://github.com/foo/bar/pull/561 emphasize safety` → PR #561 in foo/bar, focus = "emphasize safety"
...
Articulate the PR's narrative frame from a **product** perspective — what a user, operator, or stakeholder experiences, not what the code does internally:

1. **Before**: What was broken, missing, or frustrating for the user/stakeholder? (One sentence.)
2. **After**: What can they now do, see, or rely on? (One sentence, outcome-level.)
3. **Scope rationale** (only if 2+ separable-looking concerns): Why do these ship together from a product standpoint? (One sentence.)

Test: if the "Before/After" pair reads as "the code used to do X, now it does Y," rewrite it. The frame should read as "the user used to hit X, now they get Y" — even for infrastructure or refactor PRs, phrase the outcome in terms of who benefits (developer velocity, operator reliability, end-user behavior).
...
|---|---|
| Small + simple (typo, config, dep bump) | 1-2 sentences, no headers. Under ~300 characters. No Technical changes section. |
| Small + non-trivial (bugfix, behavioral change) | Product-framed summary (2-4 sentences) + `## Technical changes` as 2-4 bullets. |
| Medium feature or refactor | `## Summary` (product framing, before/after) + `## Technical changes` (concise bullet outline, 4-8 bullets) + design decisions if non-obvious. |
| Large or architecturally significant | `## Summary` (product framing) + `## Technical changes` (bulleted outline, grouped by area if needed) + up to 3-5 design-decision callouts + 1-2 sentence test summary + key docs links. Target ~100 lines, cap ~150. For PRs with many mechanisms, use a Summary-level table to list them; do NOT create an H3 subsection per mechanism. If you find yourself writing 10+ subsections, consolidate to a table. |
| Performance improvement | Product framing (what users feel) + `## Technical changes` + before/after measurements as a table. |

**Technical changes section rules** (applies whenever the section is present):

- One bullet per meaningful unit (a behavior change, a new module, a migration, a swapped dependency). Not one bullet per file.
- Phrase each bullet as a factual change with enough path/identifier for a reviewer to locate it (e.g., `Replace polling loop in worker.ts with webhook subscription`).
- No code-quality narration ("cleaner", "more maintainable"). The bots cover that lane.
- If the section would be fewer than 2 bullets, drop it — the Summary already carries the change.
...
- **Lead with product value, not mechanism**: Open with what a user, operator, or stakeholder can now do — not what was moved around in the code. The subtler failure is leading with the mechanism ("Replace the hardcoded capture block with a tiered skill") instead of the outcome ("Evidence capture now works for CLI tools and libraries, not just web apps"). Remember: humans read the top of this description to decide whether the PR is doing the right thing for the product; code-quality review is handled by the bots downstream.
- **Separate product from technical**: Keep the Summary strictly about outcome and user-facing effect. Put mechanics, file-level changes, and implementation choices under `## Technical changes` where a reviewer can skim them without them competing with the product framing.
...
- **Test plan — caller-supplied wins, otherwise only when non-obvious**: If the caller passed a `test-plan:` block or `preserve-test-plan` directive, always render a `## Test plan` section (see Step 8). Otherwise, include a test plan only when testing requires edge cases the reviewer wouldn't think of, hard-to-verify behavior, or specific setup; omit when "run the tests" is the only useful guidance. When the branch adds test files, name them with what they cover.
...
1. **Opening (product framing)** -- the narrative frame from Step 4, at the depth chosen in Step 5. Strictly product-outcome language, no implementation detail. Under a heading (e.g., `## Summary`) if the description uses any `##` headings elsewhere; a bare paragraph otherwise.
2. **Technical changes** -- required for Small+non-trivial and larger (see Step 5 sizing table). A concise `## Technical changes` bullet outline so the human reviewer can orient themselves without reading the diff line by line. Keep each bullet to a single meaningful change, not a file listing. Skip for Small+simple PRs.
3. **Design decisions / supporting sections** -- only the sections that earn their keep: why this approach over alternatives, trade-offs, tables for data, visual aids when complexity warrants. Skip empty sections entirely.
4. **Test plan** -- resolve in this order:
   - If the caller passed a `test-plan:` block, render it verbatim under a `## Test plan` heading. Preserve the caller's subheadings (`## Core scenarios`, `## Secondary scenarios`, `## Unhappy paths`) but demote them one level to `###` so they nest correctly under the `## Test plan` section. Do not paraphrase, reorder, or add content — the block is the product owner's executable plan.
   - If the caller passed `preserve-test-plan` (PR mode only), locate the existing `## Test plan` / `## How to test` section in the current PR body and copy it verbatim into the new body. If no such section exists, skip the section rather than fabricating one.
   - Otherwise, fall back to the writing-principles rule: include only when non-obvious; omit otherwise.
5. **Evidence block** -- only the preserved block from Step 3, if one exists. Do not fabricate or placeholder.
...
- If the `## Summary` describes mechanics or file paths, move those details into `## Technical changes` and keep the Summary product-framed.
- If `## Technical changes` has more than 10 bullets, group by area (short sub-bullets under 3-4 area labels) or consolidate into a table — do NOT expand into H3 subsections.
- If `## Technical changes` bullets restate what the diff already shows at a glance (one-line renames, trivial config tweaks), cut them. Keep only bullets that help a human orient faster than reading the diff.
- If `## Test plan` was caller-supplied (via `test-plan:` block or `preserve-test-plan`), do NOT compress it. The product owner authored these steps and they must survive the pass verbatim. Only the fallback "non-obvious" test plan is subject to compression — if more than 2 paragraphs, compress that one to bullets.
...
**Product-lead check.** Re-read the first sentence of the `## Summary`. If it describes what was moved around, renamed, or added ("This PR introduces three-tier autofix..."), rewrite to lead with what a user/operator/stakeholder can now do or is now spared ("Document reviews previously produced 14+ findings requiring user judgment; this PR cuts that to 4-6."). Mechanism-lead sentences belong in `## Technical changes`, not the Summary.
...
```bash
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/inc-pr-body.XXXXXX") && cat > "$BODY_FILE" <<'__INC_PR_BODY_END__' && echo "$BODY_FILE"
<the composed body markdown goes here, verbatim>
__INC_PR_BODY_END__
```

The quoted sentinel `'__INC_PR_BODY_END__'` keeps `$VAR`, backticks, `${...}`, and any literal `EOF` inside the body from being expanded or clashing with the terminator. Keep `echo "$BODY_FILE"` chained with `&&` so a failed `mktemp` or write never yields a success exit status with a path to a missing file.

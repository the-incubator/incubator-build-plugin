...
`ALL_REWRITTEN` is the **only** success signal. On `WRITE_FAILED` or `STILL_LOCAL`, say so and **retry the complete write** — re-run the full `gh pr create`/`gh pr edit` with the intended `--title`, `--body`, and `--attach` flags (safely quoted; a `./` link still in the body names the file that didn't land), then **re-capture `WRITE_RC=$?`** and re-verify. Do **not** retry `--attach` alone: an attach-only edit would append the files while leaving the intended title/body unwritten, and it would never clear the original non-zero `WRITE_RC`, so verification would stay `WRITE_FAILED` forever. On `VERIFY_UNAVAILABLE` (the read failed), do **not** report the evidence as delivered and do **not** delete the local artifacts — surface that verification couldn't run and retry the read. Once `ALL_REWRITTEN` holds, **delete the returned local artifact(s) / demo-reel run directory immediately** — demo-reel deliberately left them in place for this caller, so nothing else will clean up those (potentially sensitive) captures.

...
- **When the verification itself can't be run, the item is `blocked`.** Sometimes the pre-tag check is impossible here and now - the product source isn't in the workspace, so the git history or reachability check can't happen. The item still needs a bucket, and that bucket is `blocked`: the missing input is the verification, so name it and its owner ("confirm from git history whether the header changed; owner: any dev with the product repo"). It satisfies the `blocked` test exactly - the moment someone runs the check, the item becomes `change`, `respond`, or nothing. Never manufacture a requirement to fill the gap (step 6b), and never let an unrunnable check push an item into `change`.
...
5. Persist the final tags as `triage.md` in the analyzer output dir, next to `requirements-kickoff.md`. Its **first line must be `Analysis: <id>`**, using the `ANALYSIS_ID` the analyzer printed - an approval is only valid for the analysis it was made against, and a rerun over changed input supersedes any `triage.md` carrying a different id. Then the approved table, plus for each non-`change` item the one line its downstream action needs (the open question for `try`/`discuss`, the answer for `respond`, the dependency and owner for `blocked`, the backlog pointer for `defer`). The tags drive execution batching and the report. Nothing executes until the user approves the table.
6. Deliver the outcomes on the report, in two passes. **On approval:** badge every requirement card in `report.html` with its bucket (the BUCKET-BADGE CONTRACT comment in the report carries the exact markup); `respond` cards carry the written answer inline, `blocked` cards name the owner, `defer` cards carry the backlog pointer. Those three buckets are complete at this point - their outcome *is* the answer, the dependency, or the pointer. **After the work lands:** `change` cards gain the result with before/after evidence and `try` cards gain the approach taken plus the open question to veto. Only then is the report the closing surface; before that it records intent, so don't present it as closed.

...
- `unverifiable` → **Gate 4 BLOCK (could not verify).** The gate fail-safes to a block rather than passing silently, in any of these cases the `REASON=` / `  DRIFT[...] ` lines pinpoint:
  ...
  - **Gate tamper:** a changed schema file's *nearest* owning package defined `db:check-drift` on the base branch but no longer does in this PR - a schema PR trying to delete the very gate meant to evaluate it. This holds even when a broader ancestor package (e.g. the repo root) still defines a check: an unrelated ancestor guard can't vouch for a workspace that deleted its own. Restore the check (or split the removal into its own non-schema PR) and re-run.
